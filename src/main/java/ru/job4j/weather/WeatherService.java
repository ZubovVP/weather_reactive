...
public class WeatherService {
    ...
    public Mono<Weather> getHottest() {
        return Mono.justOrEmpty(this.weathers.values().stream().max(Comparator.comparing(Weather::getTemperature)));
    }

    public Flux<Weather> getCityGreatThen(Integer amount) {
        return Flux.fromIterable(this.weathers.values().stream().filter(e -> e.getTemperature() > amount).collect(Collectors.toList()));
    }
}
